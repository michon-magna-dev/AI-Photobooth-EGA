...
public class UploadImage : MonoBehaviour
{
    ...
    public string uploadURL = "http://192.168.1.20:5000/upload";
    //[SerializeField] ComfyImageCtr m_generatedImage;
    [SerializeField] ScreenShotEditable m_generatedImage;
    [SerializeField] string m_extractedPath;


    private void Start()
    {
        uploadURL = ConfigManager.GetInstance().GetStringValue("SERVER_URL");
    }
    ...
    {
        Texture2D texture2D = ConvertToTexture2D(capturedImage.texture);
        if (texture2D == null)
        {
        ...
        form.AddBinaryData("file", textureBytes, $"{m_generatedImage.GetLatestImageFileName()}", "image/png");
    ...
    private void OnGUI()
    {
        ...
        {
            if (GUI.Button(new Rect(250, 250, 250, 250), "Upload"))
            {
                StartUpload();
            }
        }
